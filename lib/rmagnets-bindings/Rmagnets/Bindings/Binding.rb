...
class ::Rmagnets::Bindings::Binding
  ...
  attr_configuration        :name, :view_class, 
                            :required?, 
  ...
  attr_reader :bound_instance

  ################
  #  initialize  #
  ################

  def initialize( bound_instance, binding_name, view_class = nil, & configuration_proc )
    ...
    if block_given?
      configuration_procs.push( configuration_proc )
    end
      ...
      when Array
        ...
        unless multiple_values_permitted?
          raise ::Rmagnets::Bindings::Exception::BindingInstanceInvalidTypeError
                  ...
                  'binding configuration for binding :' + name.to_s
        end
    ...
    # if binding is required and has a nil value, raise exception
		unless binding_value or optional?
			...
			raise ::Rmagnets::Bindings::Exception::BindingRequired,
			        'Binding :' + name.to_s + ' is required but not bound for binding :' + 
			        name.to_s + ' (' + @bound_instance.to_s + ').'
